...
def fast_process_image(img, raw_p, focus_positions, params, x_offset=0):
    ...
    # Адаптивный epsilon: базовый шум + малая доля от полной энергии A-скана
    # Это спасает Вермеера на границе перехода к прозрачности
    alpha_eps = 1e-4
    base_eps = 1e-6
    # adaptive_epsilon = base_eps + alpha_eps * sum_intensity[0, :]
    eps_matrix = base_eps + alpha_eps * sum_intensity[0, :]
    eps_matrix_2d = np.tile(eps_matrix, (rows, 1))

    mu_t_vermeer = np.zeros_like(I_new, dtype=np.float32)
    mask_valid = sum_shifted > 1e-12
    # Используем broadcasting для adaptive_epsilon
    mu_t_vermeer[mask_valid] = I_new[mask_valid] / (
            2.0 * delta * sum_shifted[mask_valid] + eps_matrix_2d[mask_valid]
    )

    # =========================
    # 10. Прямой экспоненциальный фиттинг (МНК)
    # =========================
    # Формируем маску только для пикселей внутри самого образца
    mask_sample = ~(mask_air)
    if mask_below is not None:
        mask_sample = mask_sample & ~(mask_below)

    # y = ln(I), x = z
    I_log = np.log(np.clip(I_new, 1e-12, None))
    x_coords = z_relative

    # Обнуляем всё вне образца, чтобы не портить суммы для МНК
    x_masked = np.where(mask_sample, x_coords, 0.0)
    y_masked = np.where(mask_sample, I_log, 0.0)

    # Векторизованный МНК по каждому столбцу (A-скану)
    N_points = np.sum(mask_sample, axis=0)
    N_points = np.clip(N_points, 1, None)  # Защита от деления на 0 для пустых столбцов
    ...
    mu_t_fit_1d = -k_slope / 2.0
    mu_t_fit_1d = np.clip(mu_t_fit_1d, 0.0, None)  # mu_t физически не может быть отрицательным

    # Растягиваем 1D массив фита (одно усредненное значение на A-скан) на всю 2D матрицу
    mu_t_fit = np.tile(mu_t_fit_1d, (rows, 1))

    # =========================
    # 10.5 Гибридное слияние (Adaptive Weighting)
    # =========================
    # Оцениваем, сколько света "долетело" до нижней четверти образца
    # Если света много -> среда прозрачная -> используем Fit

    # Ищем максимальную глубину образца для каждого столбца
    max_depths = np.max(x_masked, axis=0)

    # Энергия в нижних 20% образца
    tail_mask = mask_sample & (z_relative > 0.8 * max_depths[None, :])
    tail_energy = np.sum(I_new * tail_mask, axis=0)

    total_energy = np.sum(I_new * mask_sample, axis=0)
    total_energy = np.clip(total_energy, 1e-12, None)

    transparency_index = tail_energy / total_energy

    # Настройка границ перехода (подбирается эмпирически)
    # Если индекс < t_min (мутно) -> вес фита = 0 (работает Вермеер)
    # Если индекс > t_max (прозрачно) -> вес фита = 1 (работает Экспонента)
    t_min = 0.02
    t_max = 0.10
    ...
    # Итоговый расчет
    mu_t = mu_t_vermeer * (1.0 - fit_weight) + mu_t_fit * fit_weight
